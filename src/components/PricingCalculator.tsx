...
export default function PricingCalculator() {
  ...
  return (
    <section className="relative bg-gradient-to-b from-black via-zinc-950 to-zinc-900 text-white py-16 border-b-2 border-zinc-800 overflow-hidden" id="pricing-section">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

        <div className="text-center max-w-3xl mx-auto mb-14">
          <span className="text-xs uppercase font-sans text-red-400 font-bold tracking-widest bg-white/10 border border-white/20 px-3 py-1 rounded-full mb-6 inline-block">PRIJZEN</span>
          <h2 className="text-4xl sm:text-5xl md:text-6xl font-extrabold font-display tracking-tight text-white leading-tight mb-10">
            Eén abonnement,{" "}
            <span className="italic font-serif font-normal text-red-400">eindeloze</span>
            <br />mogelijkheden
          </h2>

          <p className="text-zinc-300 text-base sm:text-lg font-medium mb-8 leading-relaxed">
            Het is <span className="text-white font-bold italic">"je gaat nooit meer terug"</span> beter.<br />
            <span className="text-zinc-400 text-sm sm:text-base font-normal">omnitv vervangt dure Ziggo-pakketten en meerdere streamingdiensten voor één vaste prijs.</span>
          </p>
          ...
          <div className="inline-block bg-white/10 backdrop-blur border border-white/20 rounded-2xl px-8 py-6 shadow-md">
            <div className="flex items-center gap-2 justify-center mb-5">
              ...
              <span className="text-[11px] font-bold uppercase tracking-widest text-zinc-300 font-sans">
              ...
              {[
                ...
                <div key={i} className="flex items-center gap-4">
                  <div className="text-center">
                    ...
                    <span className="text-[9px] text-zinc-400 font-sans tracking-widest mt-1 block">{unit.label}</span>
                  </div>
        ...
        <div className="text-center max-w-3xl mx-auto mb-10">
          <span className="text-xs uppercase font-sans text-red-400 font-bold tracking-widest bg-zinc-800 border border-zinc-700 px-3 py-1 rounded-full mb-3 inline-block">IPTV ABONNEMENTEN</span>
          ...
          <p className="text-zinc-400 text-sm sm:text-base">Kies je periode en aantal apparaten. Geen onverwachte kosten.</p>
        </div>

        <div className="max-w-4xl mx-auto bg-zinc-900 border border-zinc-800 rounded-2xl p-6 sm:p-8 shadow-md mb-12">
          <div className="grid grid-cols-1 gap-8">
            <div className="flex flex-col gap-6">

              {/* Period tabs */}
              <div>
                <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest block mb-3 font-sans">1. Kies de abonnementsduur:</label>
                <div className="flex gap-2">
                  {periods.map((p) => (
                    <button key={p.id} onClick={() => setSelectedPeriod(p.id)}
                      ...
                      <span className="absolute top-1.5 right-1.5 bg-black text-red-400 text-[8px] font-black px-1.5 py-0.5 rounded-full"
                        style={{ display: p.bonus ? "block" : "none" }}>-50%</span>
                      <span className="block font-extrabold">{p.label}</span>
                      <span className={`text-[9px] uppercase tracking-widest font-bold block mt-0.5 ${selectedPeriod === p.id ? "text-red-100" : "text-zinc-500"}`}>
                        {p.sub}
                      </span>
                    </button>
                  ))}
              ...
              {/* Device grid */}
              <div>
                <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest block mb-3 font-sans">2. Aantal gelijktijdige schermen:</label>
                <div className="grid grid-cols-2 gap-2">
                  {["1","2","3","4"].map((dev) => (
                    <button key={dev} onClick={() => setSelectedDevices(dev as "1"|"2"|"3"|"4")}
                      ...
                      <svg className={`w-5 h-5 ${selectedDevices === dev ? "text-white" : "text-zinc-500"}`} fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          ...
          {/* Premium VIP */}
          <div className="relative rounded-2xl border border-red-600 bg-gradient-to-b from-red-700 to-red-900 p-8 min-h-[560px] flex flex-col shadow-xl">
            <div className="flex justify-center mb-4"><span className="bg-black text-red-300 text-[10px] font-extrabold px-5 py-1.5 rounded-full uppercase tracking-widest shadow-md">BESTE DEAL</span></div>
            <h3 className="text-3xl font-extrabold font-display text-white mb-1">✦ PREMIUM VIP +</h3>
            <p className="text-red-200/70 text-xs font-sans uppercase tracking-widest mb-4">12+3 MAANDEN</p>
            <div className="mb-4">
              <span onClick={() => openWhatsApp("VIP", vipPrice)}
                className="text-5xl font-black text-white font-display cursor-pointer hover:text-red-200 transition-colors">
                €{vipPrice.toFixed(2).replace(".",",")}
              </span>
            </div>
            <p className="text-red-100/70 text-xs mb-5 flex items-center gap-1">
            ...
            <div className="flex flex-col gap-2.5 mb-6 flex-1">
              {[
                ...
                <div key={feature} className="flex items-start gap-2.5 text-xs text-red-50">
                  <div className="w-4 h-4 rounded-full bg-white/15 border border-white/30 flex items-center justify-center flex-shrink-0 mt-0.5">
            ...
            <button onClick={() => openWhatsApp("VIP", vipPrice)}
              className="w-full py-3.5 rounded-xl bg-black hover:bg-zinc-900 text-red-400 border border-red-800 font-bold text-sm uppercase tracking-wide transition-all cursor-pointer mt-auto shadow-lg">
              Bestel Nu &rarr;
            </button>
          </div>

          {/* Basis */}
          <div className="relative rounded-2xl border border-zinc-700 bg-gradient-to-b from-zinc-900 to-zinc-950 p-8 min-h-[560px] flex flex-col shadow-sm">
            <h3 className="text-3xl font-extrabold font-display text-white mb-1 mt-2">Basis</h3>
            <p className="text-zinc-400 text-xs font-sans uppercase tracking-widest mb-4">12 MAANDEN</p>
            <div className="mb-4">
              <span onClick={() => openWhatsApp("Basis", normalPrice)}
                className="text-5xl font-black text-white cursor-pointer hover:text-red-400 transition-colors" style={{ fontFamily: "'Sour Gummy', cursive" }}>
            ...
            <button onClick={() => openWhatsApp("Basis", normalPrice)}
              className="w-full py-3.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 border border-zinc-600 text-zinc-200 font-bold text-sm uppercase tracking-wide transition-all cursor-pointer mt-auto">
              Bestel Nu &rarr;
            </button>
          </div>
        </div>
        ...
        {checkoutModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
            <motion.div initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} exit={{opacity:0,scale:0.95}}
              className="relative w-full max-w-lg bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl p-6 sm:p-8 text-white max-h-[90vh] overflow-y-auto">
              <button onClick={() => setCheckoutModalOpen(false)} className="absolute top-4 right-4 px-3 py-1 rounded-lg bg-zinc-800 text-zinc-400 hover:text-white text-xs font-bold">Sluiten</button>

              <div className="flex items-center justify-between mb-6 pb-4 border-b border-zinc-800 text-xs text-zinc-500 font-sans">
                {["details","payment","processing","success"].map((step,i)=>(
                  <span key={step} className={checkoutStep===step?"text-white font-bold":"text-zinc-600"}>
              ...
              {checkoutStep==="details" && (
                <div className="text-left">
                  ...
                  <p className="text-xs text-zinc-400 mb-5">We sturen je inloggegevens direct naar je e-mail.</p>
                  <div className="flex flex-col gap-4">
                    <div>
                      <label className="text-xs font-bold text-zinc-400 block mb-1.5 font-sans">E-mailadres:</label>
                      <input type="email" required value={email} onChange={e=>setEmail(e.target.value)} placeholder="jouw@email.nl"
                        className="w-full bg-zinc-800 border border-zinc-700 rounded-xl py-3 px-4 text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-red-600"/>
                    </div>
                    <div>
                      <label className="text-xs font-bold text-zinc-400 block mb-1.5 font-sans">Apparaat:</label>
                      <select value={targetDevice} onChange={e=>setTargetDevice(e.target.value)} className="w-full bg-zinc-800 border border-zinc-700 rounded-xl py-3 px-4 text-sm text-white focus:outline-none">
                    ...
                    <div className="p-3.5 rounded-xl bg-zinc-800 border border-zinc-700 text-xs text-zinc-400">
                    ...
                    <button onClick={() => { if(email.includes("@")){setCheckoutStep("payment");}else{alert("Vul een geldig e-mailadres in!");}}}
                      className="w-full mt-4 py-3.5 rounded-xl bg-red-700 text-white hover:bg-red-800 font-bold text-sm text-center transition-colors">
                  ...
                  <form onSubmit={handleProcessPayment} className="flex flex-col gap-4 mt-5">
                    {[{id:"ideal",label:"iDEAL (Nederland)",badge:"Populair"},{id:"bancontact",label:"Bancontact (België)",badge:""},{id:"paypal",label:"PayPal",badge:""},{id:"cc",label:"Creditcard (Visa/MC)",badge:""}].map(pm=>(
                      <div key={pm.id} onClick={()=>setPaymentMethod(pm.id)}
                        className={`p-4 rounded-xl border cursor-pointer transition-all ${paymentMethod===pm.id?"bg-zinc-800 border-red-600":"bg-zinc-800 border-zinc-700 hover:border-zinc-600"}`}>
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-bold flex items-center gap-2 text-white">
                            <span className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${paymentMethod===pm.id?"border-red-500":"border-zinc-500"}`}>
                              {paymentMethod===pm.id&&<span className="w-2 h-2 bg-red-500 rounded-full"/>}
                            </span>
                            {pm.label}
                          </span>
                          {pm.badge&&<span className="bg-red-700 text-white text-[9px] font-bold px-1.5 py-0.5 rounded">{pm.badge}</span>}
                        </div>
                        {pm.id==="ideal"&&paymentMethod==="ideal"&&(
                          <select value={selectedBank} onChange={e=>setSelectedBank(e.target.value)} className="mt-3 w-full bg-zinc-700 border border-zinc-600 rounded-lg py-2 px-3 text-xs text-white">
                    ...
                    <div className="flex gap-2">
                      <button type="button" onClick={()=>setCheckoutStep("details")} className="w-1/3 py-3.5 border border-zinc-700 rounded-xl text-zinc-400 font-bold text-sm">Terug</button>
                      <button type="submit" className="w-2/3 py-3.5 rounded-xl bg-red-700 text-white hover:bg-red-800 font-bold text-sm transition-colors">Voldoe €{calculatedTotalPrice.toFixed(2)}</button>
                    </div>
                  </form>
                </div>
              )}
              ...
              {checkoutStep==="processing" && (
                <div className="py-8 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-xs text-zinc-400 max-w-sm">Verbinding met bank. We activeren je IPTV lijn.</p>
                </div>
              )}

              {checkoutStep==="success" && (
                <div className="text-left py-2">
                  <div className="w-12 h-12 rounded-full bg-red-950 border border-red-800 text-red-400 flex items-center justify-center mb-4 mx-auto">
                    <Check className="w-6 h-6 stroke-[3]"/>
                  </div>
                  <h3 className="text-xl font-bold text-white text-center mb-1">Betaling Geslaagd!</h3>
                  <p className="text-xs text-zinc-400 text-center mb-5">Jouw omnitv lijn is per direct geactiveerd.</p>
                  <div className="bg-zinc-800 border border-zinc-700 rounded-xl p-4 flex flex-col gap-3 text-xs mb-5">
                    {[{label:"Host:",val:generatedLine.host},{label:"Gebruikersnaam:",val:generatedLine.username},{label:"Wachtwoord:",val:generatedLine.password}].map((item,i)=>(
                      <div key={i} className={`flex justify-between items-center py-1 ${i<2?"border-b border-zinc-700":""}`}>
                        <span className="text-zinc-500">{item.label}</span>
                        <span className="font-sans text-white font-bold select-all">{item.val}</span>
                      </div>
                    ))}
                    <div className="flex justify-between items-center py-1 border-t border-zinc-700">
                      <span className="text-zinc-500">M3U URL:</span>
                      <button onClick={()=>navigator.clipboard.writeText(generatedLine.m3uUrl).then(()=>alert("Gekopieerd!"))} className="text-red-400 font-bold flex items-center gap-1 font-sans text-[10px] hover:text-red-300">
                        <Copy className="w-3 h-3"/> Kopieer
                      </button>
                    </div>
                  </div>
                  <button onClick={()=>setCheckoutModalOpen(false)} className="w-full py-3.5 rounded-xl bg-red-700 text-white hover:bg-red-800 font-bold text-sm transition-colors">
